'use client';

import { useState, useMemo, useCallback, useEffect } from 'react';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
  PointerSensor,
  useSensor,
  useSensors,
  DragEndEvent,
} from '@dnd-kit/core';
import {
  arrayMove,
  SortableContext,
  sortableKeyboardCoordinates,
  verticalListSortingStrategy,
} from '@dnd-kit/sortable';
import { format, addSeconds, parse } from 'date-fns';
import { fr } from 'date-fns/locale';
import { Clock, AlertTriangle } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { ScrollArea } from '@/components/ui/scroll-area';
import { Skeleton } from '@/components/ui/skeleton';
import { RundownItem } from './RundownItem';
import { AddItemDialog } from './AddItemDialog';
import { CollaborationAwareness } from '@/components/collaboration';
import { useYRundown, RundownItemData, CollaborationUser } from '@/lib/collaboration';
import { trpc } from '@/lib/trpc/client';
import { cn } from '@/lib/utils';
import { useAuth } from '@/components/auth/AuthProvider';

interface CollaborativeRundownEditorProps {
  rundownId: string;
}

const statusColors = {
  DRAFT: 'bg-gray-100 text-gray-700',
  READY: 'bg-green-100 text-green-700',
  ON_AIR: 'bg-red-100 text-red-700',
  ARCHIVED: 'bg-blue-100 text-blue-700',
};

export function CollaborativeRundownEditor({ rundownId }: CollaborativeRundownEditorProps) {
  const { user: authUser } = useAuth();

  // Initial data fetch from server
  const { data: rundown, isLoading } = trpc.rundown.get.useQuery({
    id: rundownId,
  });

  const utils = trpc.useUtils();

  // Batch update to server
  const batchUpdate = trpc.rundown.batchUpdate.useMutation({
    onSuccess: () => {
      utils.rundown.get.invalidate({ id: rundownId });
    },
  });

  // Create collaboration user from auth
  const collaborationUser: CollaborationUser = useMemo(() => ({
    id: authUser?.userId || 'anonymous',
    name: authUser?.email?.split('@')[0] || 'Anonyme',
    color: '',
  }), [authUser]);

  // Transform server data to collaboration format
  const initialItems: RundownItemData[] = useMemo(() => {
    if (!rundown) return [];
    return rundown.items.map((item) => ({
      id: item.id,
      type: item.type,
      title: item.title,
      duration: item.duration,
      position: item.position,
      status: item.status,
      notes: item.notes || undefined,
      storyId: item.storyId || undefined,
      assigneeId: item.assigneeId || undefined,
    }));
  }, [rundown]);

  const initialMeta = useMemo(() => ({
    status: rundown?.status || 'DRAFT',
    notes: rundown?.notes || undefined,
  }), [rundown]);

  // Sync to server callback
  const handleSyncToServer = useCallback((items: RundownItemData[], meta: any) => {
    if (!rundown) return;

    batchUpdate.mutate({
      rundownId,
      items: items.map((item, index) => ({
        ...item,
        position: index,
      })),
      status: meta.status,
      notes: meta.notes,
    });
  }, [rundownId, rundown, batchUpdate]);

  // Yjs collaboration hook
  const {
    items,
    meta,
    isConnected,
    isSynced,
    connectedUsers,
    addItem,
    updateItem,
    deleteItem,
    reorderItems,
    setCursor,
  } = useYRundown({
    rundownId,
    user: collaborationUser,
    initialItems,
    initialMeta,
    onSyncToServer: handleSyncToServer,
  });

  // Use collaborative items or fall back to server data
  const displayItems = items.length > 0 ? items : initialItems;

  const sensors = useSensors(
    useSensor(PointerSensor),
    useSensor(KeyboardSensor, {
      coordinateGetter: sortableKeyboardCoordinates,
    })
  );

  // Calculate timing
  const timing = useMemo(() => {
    const baseTime = parse('12:00', 'HH:mm', new Date());
    let currentTime = baseTime;
    const startTimes: Record<string, string> = {};
    let totalDuration = 0;

    displayItems.forEach((item) => {
      startTimes[item.id] = format(currentTime, 'HH:mm:ss');
      currentTime = addSeconds(currentTime, item.duration);
      totalDuration += item.duration;
    });

    return { startTimes, totalDuration };
  }, [displayItems]);

  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      const { active, over } = event;

      if (over && active.id !== over.id) {
        const oldIndex = displayItems.findIndex((i) => i.id === active.id);
        const newIndex = displayItems.findIndex((i) => i.id === over.id);
        reorderItems(oldIndex, newIndex);
      }
    },
    [displayItems, reorderItems]
  );

  const handleStatusChange = useCallback(
    (itemId: string, status: RundownItemData['status']) => {
      updateItem(itemId, { status });
    },
    [updateItem]
  );

  const handleDeleteItem = useCallback(
    (itemId: string) => {
      if (confirm('Supprimer cet element ?')) {
        deleteItem(itemId);
      }
    },
    [deleteItem]
  );

  const handleItemFocus = useCallback(
    (itemId: string) => {
      setCursor(itemId);
    },
    [setCursor]
  );

  const handleItemBlur = useCallback(() => {
    setCursor(null);
  }, [setCursor]);

  const formatTotalDuration = (seconds: number) => {
    const mins = Math.floor(seconds / 60);
    const secs = seconds % 60;
    return `${mins}:${secs.toString().padStart(2, '0')}`;
  };

  if (isLoading) {
    return (
      <div className="flex flex-col h-full">
        <div className="p-4 border-b">
          <Skeleton className="h-8 w-3/4 mb-2" />
          <Skeleton className="h-6 w-1/2" />
        </div>
        <div className="p-4 space-y-3">
          {[...Array(5)].map((_, i) => (
            <Skeleton key={i} className="h-16 w-full" />
          ))}
        </div>
      </div>
    );
  }

  if (!rundown) {
    return (
      <div className="flex items-center justify-center h-full text-gray-500">
        Conducteur non trouve
      </div>
    );
  }

  const targetDuration = rundown.show.defaultDuration * 60;
  const durationDiff = timing.totalDuration - targetDuration;
  const isOvertime = durationDiff > 0;
  const isUndertime = durationDiff < -60;

  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="p-4 border-b bg-white">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h2 className="text-xl font-bold">{rundown.show.name}</h2>
            <p className="text-sm text-gray-500">
              {format(new Date(rundown.date), 'EEEE d MMMM yyyy', { locale: fr })}
            </p>
          </div>
          <div className="flex items-center gap-4">
            {/* Collaboration awareness */}
            <CollaborationAwareness
              isConnected={isConnected}
              isSynced={isSynced}
              connectedUsers={connectedUsers}
              currentUser={collaborationUser}
            />
            <Badge className={statusColors[meta.status || rundown.status]}>
              {meta.status === 'DRAFT' && 'Brouillon'}
              {meta.status === 'READY' && 'Pret'}
              {meta.status === 'ON_AIR' && 'A l\'antenne'}
              {meta.status === 'ARCHIVED' && 'Archive'}
            </Badge>
          </div>
        </div>

        {/* Timing bar */}
        <div
          className={cn(
            'flex items-center gap-4 p-3 rounded-lg',
            isOvertime && 'bg-red-50',
            isUndertime && 'bg-orange-50',
            !isOvertime && !isUndertime && 'bg-gray-50'
          )}
        >
          <Clock className="h-5 w-5 text-gray-600" />
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <span className="font-mono text-lg">
                {formatTotalDuration(timing.totalDuration)}
              </span>
              <span className="text-gray-500">/</span>
              <span className="font-mono text-gray-500">
                {rundown.show.defaultDuration}:00
              </span>
            </div>
          </div>
          {(isOvertime || isUndertime) && (
            <div
              className={cn(
                'flex items-center gap-1 text-sm font-medium',
                isOvertime ? 'text-red-600' : 'text-orange-600'
              )}
            >
              <AlertTriangle className="h-4 w-4" />
              {isOvertime ? '+' : ''}
              {formatTotalDuration(Math.abs(durationDiff))}
            </div>
          )}
        </div>
      </div>

      {/* Items list */}
      <ScrollArea className="flex-1">
        <div className="p-4">
          <DndContext
            sensors={sensors}
            collisionDetection={closestCenter}
            onDragEnd={handleDragEnd}
          >
            <SortableContext
              items={displayItems.map((i) => i.id)}
              strategy={verticalListSortingStrategy}
            >
              <div className="space-y-2">
                {displayItems.map((item) => (
                  <RundownItem
                    key={item.id}
                    item={{
                      ...item,
                      notes: item.notes || null,
                      storyId: item.storyId || null,
                      assigneeId: item.assigneeId || null,
                      story: null,
                      assignee: null,
                      rundownId,
                      createdAt: new Date(),
                      updatedAt: new Date(),
                      media: [],
                    }}
                    startTime={timing.startTimes[item.id] || '--:--:--'}
                    onDelete={() => handleDeleteItem(item.id)}
                    onStatusChange={(status) => handleStatusChange(item.id, status)}
                    onFocus={() => handleItemFocus(item.id)}
                    onBlur={handleItemBlur}
                  />
                ))}
              </div>
            </SortableContext>
          </DndContext>

          {/* Add item */}
          <div className="mt-4 flex justify-center">
            <AddItemDialog
              rundownId={rundownId}
              onAddItem={(item) => {
                addItem({
                  id: `temp-${Date.now()}`,
                  type: item.type,
                  title: item.title,
                  duration: item.duration,
                  status: 'PENDING',
                  storyId: item.storyId,
                });
              }}
            />
          </div>
        </div>
      </ScrollArea>
    </div>
  );
}
